...
exports.getFrdsLastMessage = function (data, callback) {
    let friendsList = data.friends_list;
    const user_id = data.user_id;
    async.waterfall([
        function (cb) {
            var sql = 'SELECT * FROM `messages` WHERE `user_from_id`=? OR `user_to_id`=? ORDER BY `messages`.`id`';
            connection.query(sql, [user_id, user_id], (error, messages) => {
                if (error) {
                    cb({
                        message: constants.responseMessages.ERROR_IN_EXECUTION,
                        status: constants.responseFlags.ERROR_IN_EXECUTION,
                    ...
                    if (messages.length > 0) {
                        friendsList.forEach(element => {
                            ...
                            messages.forEach((element_message) => {
                                if (element_message.user_from_id == user_id &&  element_message.user_to_id == element.user_id 
                                    ||
                                    element_message.user_from_id == element.user_id &&  element_message.user_to_id == user_id
                                    ) {
                                        element.message = element_message.type == 0 ? element_message.message : ((element_message.type == 1 || element_message.type == 3) ? element_message.media : element_message.audio);
                                        element.type = element_message.type;
                                        is_message = true;
                                        return false
                                    }
                            })
                    ...
                    cb(null, {user_id, friends_list: friendsList});
                }
            });
        }
    ], function (error, result) {
                ...
                checkIfUserSendMessage(sender_id, reciever_id, function (err, message_data) {
                    if (err) {
                        ...
                        if (checkIfUserSend) {
                            ...
                            var sql = "INSERT INTO `messages` (`user_from_id`,`user_to_id`,`message`, `type`, `created_at`,`updated_at`, `message_is_read`) VALUES(?,?,?,?,?,?,?)";
                            var query = connection.query(sql, [sender_id, reciever_id, message, 0, datetime, datetime, 1], (error, user) => {
                                if (error) {
                                    console.log(error);
                                    cb({
                                        message: constants.responseMessages.ERROR_IN_EXECUTION,
            ...
            if (!user || (user && user.length === 0)) {
                ...
                checkIfUserSendMessage(sender_id, reciever_id, function (err, message_data) {
                    if (err) {
                        ...
                        if (checkIfUserSend) {
                            var sql = 'INSERT INTO `messages` (`user_from_id`,`user_to_id`,`message`, `type`, `media`,`created_at`,`updated_at`, `message_is_read`) VALUES(?,?,?,?,?,?,?,?)'
                            var query = connection.query(sql, [sender_id, reciever_id, "", 3, data.file, datetime, datetime, 1], (error, user) => {
                ...
                checkIfUserSendMessage(sender_id, reciever_id, function (err, message_data) {
                    ...
                    if (err) {
                        ...
                        if (checkIfUserSend) {
                            ...
                            var sql = 'INSERT INTO `messages` (`user_from_id`,`user_to_id`,`message`, `type`, `media`, `audio`, `created_at`,`updated_at`, `message_is_read`) VALUES(?,?,?,?,?,?,?,?,?)'
                            var query = connection.query(sql, [sender_id, reciever_id, "", 2, "", data.blob, datetime, datetime, 1], (error, user) => {
                                if (error) {
                                    console.log(error);
                                    cb({
                                        message: constants.responseMessages.ERROR_IN_EXECUTION,
                ...
                checkIfUserSendMessage(sender_id, reciever_id, function (err, message_data) {
                    ...
                    if (err) {
                        ...
                        if (checkIfUserSend) {
                            ...
                            var sql = 'INSERT INTO `messages` (`user_from_id`,`user_to_id`,`message`, `type`, `media`,`created_at`,`updated_at`, `message_is_read`) VALUES(?,?,?,?,?,?,?,?)'
                            var query = connection.query(sql, [sender_id, reciever_id, "", 1, data.file, datetime, datetime, 1], (error, user) => {
                                if (error) {
                                    console.log(error);
                                    cb({
                                        message: constants.responseMessages.ERROR_IN_EXECUTION,
